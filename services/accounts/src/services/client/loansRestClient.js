  ...
  constructor() {
    ...
    axiosRetry(this.client, {
      ...
      retryDelay: (retryCount) => {
        ...
        logger.debug(`Retrying loans service call, attempt ${retryCount}, delay: ${delay}ms`);
        return delay;
      },
    ...
    // Create circuit breaker for loans service
    this.circuitBreaker = circuitBreakerFactory.createBreaker(
      'loans-service', 
      this._makeRequest.bind(this),
      {
        timeout: parseInt(process.env.LOANS_SERVICE_TIMEOUT || '3000'),
  ...
  async fetchLoan(mobileNumber, correlationId) {
    try {
      ...
      if (error.response && error.response.status === 500) {
        logger.warn(`Loans not found for mobile number ${mobileNumber}`);
        throw new Error(`Loans not found for mobile number ${mobileNumber}`);
      }
      if (error.message.includes('circuit breaker')) {
        logger.error('Loans service circuit breaker triggered:', {
          message: error.message,
          mobileNumber,
          correlationId
        });
      } else {
        logger.error('Error calling Loans service after retries:', {
          message: error.message,
          mobileNumber,
          correlationId
        });
